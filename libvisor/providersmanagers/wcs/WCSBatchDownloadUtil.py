...
class WCSDownloadWorkerThread(DownloadWorkerThread):
    ...
    def __init__(
        self, capabilitiesURL, times, layerName, boundingBox, parent=None, jobName=None
    ):
        """
    ...
    def run(self):
        ...
        for moment in self.times:
            ...
            mapDownloadURL = parser.generateURLForGeoTIFF(
                self.layerName, moment, self.boundingBox
            )
            layer = parser.generateLayerFromGeoTIFFURL(
                mapDownloadURL, moment + "_" + self.layerName
            )
