...
class WMSparser(QObject):
    ...
    def __init__(self, urlXML):
        ...
        self.WMSValueRangeCheckURL = (
            str(baseResourceURL[0]) + "?item=minmax"
            "&request=GetMetadata"
            "&crs=EPSG:4326"
            "&srs=EPSG:4326"
            "&dpiMode=7"
    ...
    def getMapInfoFromCapabilities(self):
        ...
        nombre = self.tree.find(
            ".//{0}Layer/{0}Layer/{0}Title".format(self.namespace)
        ).text
        getMapAttrib = self.tree.find(
            ".//{0}Capability/{0}Request/{0}GetMap/{0}DCPType/{0}HTTP/{0}Get/{0}OnlineResource".format(
                self.namespace
            )
        ).attrib
        url = getMapAttrib[self.xlink + "href"]
        search = self.tree.findall(
            ".//{0}Layer/{0}Layer/{0}Layer".format(self.namespace)
        )
        ...
        for result in search:
            ...
            layerObject.setAbstract(
                result.find(".//{0}Abstract".format(self.namespace)).text
            )
            BoundingBoxInfo = (
                result.find(".//{0}BoundingBox".format(self.namespace))
            ).attrib
            ...
            try:
                preFormatTimes = result.find(
                    ".//{0}Dimension[@name='time']".format(self.namespace)
                ).text
            ...
            for style in styles:
                ...
                styleObject.setName(
                    style.find(".//{0}Name".format(self.namespace)).text
                )
                styleObject.setAbstract(
                    style.find(".//{0}Abstract".format(self.namespace)).text
                )
                tags = style.find(
                    ".//{0}LegendURL//{0}OnlineResource".format(self.namespace)
                ).attrib
    ...
    def createMapLayer(
        self, mapLayerName, layerStyleName, boundingBox, layerTime="", minMaxRange=None
    ):
        ...
        finalUrl = self.baseWMSUrl.format(
            layer=mapLayerName, style=layerStyleName, url=self.mapInfo.getURL()
        )
        ...
        resultLayer.dataProvider().setDataSourceUri(
            ...
            + "?TIME={time}%26COLORSCALERANGE={scale}%26BBOX={bbox}".format(
                time=layerTime, scale=rasterMinMaxValues, bbox=str(boundingBox)
            )
        )
    ...
    def getMinMaxRasterValuesFromTimeRange(
        self, mapLayerName, layerStyleName, listOfTimes, boundingBox
    ):
        ...
        for moment in listOfTimes:
            ...
            thread = threading.Thread(
                target=self._getRangeValuesFromJSONPath, args=(pathForJSON,)
            )
