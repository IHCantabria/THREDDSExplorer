...
class WMSDownloadWorkerThread(DownloadWorkerThread):
    ...
    def __init__(
        self, capabilitiesURL, times, layerName, style, bbox, parent=None, jobName=None
    ):
        """
        ...
        self.parser.singleRangeChecked.connect(
            self.WMSSingleValueRangeProcessed.emit, Qt.DirectConnection
        )
    ...
    def run(self):
        ...
        valuesRange = self.parser.getMinMaxRasterValuesFromTimeRange(
            self.layerName, self.style, self.times, self.bbox
        )
        ...
        for moment in times:
            ...
            self.parser.createMapLayer(
                self.layerName,
                self.style,
                self.bbox,
                layerTime=moment,
                minMaxRange=valuesRange,
            )
            ...
            if layer is not None and layer.isValid() == True:
                ...
                (self.getLayerDict())[
                    datetime.strptime(moment, self.inTimeFormat)
