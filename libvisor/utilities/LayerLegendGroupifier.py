...
class LayerGroupifier(QObject):
    ...
    errorsFound = pyqtSignal(
        int
    )  # Number of errors found during the groupify operation
    groupifyComplete = pyqtSignal(
        QgsLayerTreeGroup, list
    )  # Emits the created group and the list of layers assigned to it
    ...
    def getGeneratedGroup(self):
        ...
        with self.groupAssignmentLock:
            if self.generatedGroup is None:
                ...
                self.generatedGroup.setIsMutuallyExclusive(
                    self.singleLayerSelectionModeInGroup
                )
                self.generatedGroup.addedChildren.connect(
                    self.onChildrenAddedToNodeGroup
                )
    ...
    def onChildrenAddedToNodeGroup(self, node, indexFrom, indexTo):
        ...
        if self.layersToBeGrouped == self.layersAddedToGroups:
            ...
            self.groupifyComplete.emit(
                self.generatedGroup, self.correctlyRegisteredLayers
            )
